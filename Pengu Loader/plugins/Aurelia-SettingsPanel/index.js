...
(function initSettingsPanel() {
  ...
  function getCSSRules() {
    ...
    @keyframes aureliaWarningPulse {
      ...
      50%  { filter: drop-shadow(0 0 6px rgba(0, 242, 255, 0.90)) drop-shadow(0 0 12px rgba(163, 51, 255, 0.45)); opacity: 1.00; }
    ...
    #aurelia-diagnostics-body::-webkit-scrollbar-thumb {
      background: linear-gradient(to bottom, rgba(163, 51, 255, 0.22), rgba(0, 242, 255, 0.85));
  ...
  function startBadgeObserver() {
    ...
    // Re-apply badges when the Golden Aurelia nav item is injected by AURELIA-UI (or recreated by Ember).
    const tryApply = () => {
      try {
        applyErrorBadges();
      } catch (e) {}
  ...
  function createSettingsFlyout(navItem) {
    ...
    brandCard.innerHTML = `
      <div class="settings-brand-kicker">aurelia fork</div>
    ...
    thresholdFill.style.background = "linear-gradient(to left, #a333ff, #00f2ff)";
    ...
    timeoutFill.style.background = "linear-gradient(to left, #a333ff, #00f2ff)";
    ...
    const updateHoverEffects = () => {
      if (isHovered || isDragging) {
        fill.style.background = isDragging
          ? 'linear-gradient(to right, #a333ff, #00f2ff)'
          : 'linear-gradient(to right, #a333ff 0%, #00f2ff 56%, #ffffff 100%)';
        button.style.backgroundPosition = isDragging ? '0 -60px' : '0 -30px';
      } else {
        fill.style.background = 'linear-gradient(to left, #a333ff, #00f2ff)';
    ...
    if (errors.length === 0) {
      ...
      const itemsHtml = errors
        .map((e, idx) => {
          ...
          return `
            <div style="border:1px solid rgba(163,51,255,0.55); background: rgba(1,10,19,0.35); padding:8px; margin-bottom:8px;">
  ...
  // Listen for open settings event from AURELIA-UI
