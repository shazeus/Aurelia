...
(function enableLockedSkinPreview() {
  ...
  function handleSkipBaseSkin(payload) {
    ...
    log.info("received base skin skip request from aurelia");
  }
              ...
              if (eventData["uri"] == "/lol-champ-select/v1/skin-selector-info") {
                ...
                if (eventData["data"]?.["selectedSkinId"] != 0) {
                  if (Date.now() - lastBaseSkinSkipRequest < BASE_SKIN_SKIP_REQUEST_TIME_WINDOW_MS) {
                    ...
                    log.info("not skipping base skin: no request received from aurelia (in time)");
                  }
                }
              }
            }
  ...
  const INLINE_RULES = `
    ...
    /* Prevent active state styling for Golden Aurelia */
    lol-uikit-navigation-item.menu_item_Golden\\ Aurelia .section.active::before,
    lol-uikit-navigation-item.menu_item_Golden\\ Aurelia .section.active::after,
    lol-uikit-navigation-item.menu_item_Golden\\ Aurelia .section.active,
    lol-uikit-navigation-item.menu_item_Golden\\ Aurelia .section.active .section-glow,
    lol-uikit-navigation-item.menu_item_Golden\\ Aurelia .section.active .section-glow-container {
    ...
    lol-uikit-navigation-item.menu_item_Golden\\ Aurelia .section:hover::after {
      opacity: 0 !important;
      background: none !important;
      background-image: none !important;
    }
    ...
    .skin-selection-carousel .skin-selection-item .lpp-skin-border {
      ...
      border-image-source: linear-gradient(0deg, #4f4f54 0%, #a333ff 50%, #29272b 100%);
    ...
    .skin-selection-carousel .skin-selection-item.skin-carousel-offset-2 .lpp-skin-border {
      border: 2px solid transparent;
      border-image-source: linear-gradient(0deg, #a333ff 0%, #00f2ff 44%, #a07b32 59%, #785a28 100%);
      border-image-slice: 1;
  ...
  function attachGoldenAureliaListeners(navItem) {
    ...
    navItem.addEventListener(
      ...
      (e) => {
        ...
        if (lastActiveNavItem) {
          lastActiveNavItem.setAttribute("aureliaLastActive", true);
        }

        // Dispatch event to open settings panel
        const event = new CustomEvent("aurelia-open-settings", {
          detail: { navItem: navItem },
          bubbles: true,
          cancelable: true,
        });
        window.dispatchEvent(event);
        log.info("Dispatched aurelia-open-settings event from Golden Aurelia button");
      },
      true
      ...
      if (section && !section.dataset.lppDiscordHandler) {
        ...
        section.addEventListener(
          ...
          (e) => {
            ...
            window.dispatchEvent(event);
            log.info(
              "Dispatched aurelia-open-settings event from Golden Aurelia section"
            );
  ...
  function injectGoldenAureliaNavItem() {
    const rightNavMenu = document.querySelector(".right-nav-menu");
    if (!rightNavMenu) {
      return false;
    }

    // Check if Golden Aurelia item already exists by checking for the golden_aurelia.png image
    const existingItem = rightNavMenu.querySelector(
      'lol-uikit-navigation-item .menu-item-icon[style*="golden_aurelia.png"]'
    );
    if (existingItem) {
      const navItem = existingItem.closest("lol-uikit-navigation-item");
      if (navItem) {
        attachGoldenAureliaListeners(navItem);
      }
      return true;
    }
    ...
    navItem.className =
      "main-navigation-menu-item menu_item_Golden Aurelia ember-view";
    ...
    icon.style.webkitMaskImage = `url(http://127.0.0.1:${window.__aureliaBridge ? window.__aureliaBridge.port : 50000}/asset/golden_aurelia.png)`;
    ...
    // Add separator after the Golden Aurelia item
    ...
    log.info("Golden Aurelia navigation item injected");
    return true;
  }

  function setupNavObserver() {
    // Try to inject immediately
    if (injectGoldenAureliaNavItem()) {
      return;
    }
