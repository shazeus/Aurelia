...
(function initSettingsPanel() {
  ...
  function getCSSRules() {
    return `
    @keyframes aureliaWarningPulse {
      0%   { filter: drop-shadow(0 0 0 rgba(0, 242, 255, 0.00)) drop-shadow(0 0 0 rgba(0, 242, 255, 0.00)); opacity: 0.95; }
      50%  { filter: drop-shadow(0 0 6px rgba(0, 242, 255, 0.90)) drop-shadow(0 0 12px rgba(0, 242, 255, 0.45)); opacity: 1.00; }
      100% { filter: drop-shadow(0 0 0 rgba(0, 242, 255, 0.00)) drop-shadow(0 0 0 rgba(0, 242, 255, 0.00)); opacity: 0.95; }
    }

    .aurelia-warning-glow {
      animation: aureliaWarningPulse 1.35s ease-in-out infinite;
      will-change: filter, opacity;
    }

    @font-face {
      font-family: "Beaufort for LOL";
      src: url("http://127.0.0.1:${window.__aureliaBridge ? window.__aureliaBridge.port : 50000}/asset/BeaufortforLOL-Regular.ttf") format("truetype");
      font-weight: normal;
      font-style: normal;
      font-display: swap;
    }
    ...
    @font-face {
      font-family: "Beaufort for LOL";
      src: url("http://127.0.0.1:${window.__aureliaBridge ? window.__aureliaBridge.port : 50000}/asset/BeaufortforLOL-Bold.ttf") format("truetype");
      font-weight: bold;
      font-style: normal;
      font-display: swap;
    }
    ...
    #aurelia-diagnostics-body {
      scrollbar-width: thin;
      scrollbar-color: #a333ff rgba(0, 0, 0, 0.25);
    }

    #aurelia-diagnostics-body::-webkit-scrollbar {
      width: 10px;
    }

    #aurelia-diagnostics-body::-webkit-scrollbar:horizontal {
      display: none !important;
      height: 0 !important;
    }

    #aurelia-diagnostics-body::-webkit-scrollbar-track {
      background: rgba(0, 0, 0, 0.25);
      border-left: 1px solid rgba(163, 51, 255, 0.55);
    }

    #aurelia-diagnostics-body::-webkit-scrollbar-thumb {
      background: linear-gradient(to bottom, rgba(163, 51, 255, 0.22), rgba(163, 51, 255, 0.85));
      border: 1px solid rgba(163, 51, 255, 0.95);
      border-radius: 10px;
      min-height: 28px;
    }

    #aurelia-diagnostics-body::-webkit-scrollbar-thumb:hover {
      background: linear-gradient(to bottom, rgba(163, 51, 255, 0.32), rgba(163, 51, 255, 0.95));
    }

    #aurelia-diagnostics-body::-webkit-scrollbar-corner {
    ...
    #${FLYOUT_ID} .lc-flyout-content {
      ...
      background-image:
        radial-gradient(circle at 18% 0%, rgba(163, 51, 255, 0.10), transparent 32%),
        linear-gradient(180deg, rgba(10, 20, 40, 0.92), #010a13 44%, #010a13) !important;
      border-radius: 0 !important;
      padding: 20px !important;
      width: 100% !important;
      box-sizing: border-box !important;
      border: 1px solid rgba(163, 51, 255, 0.40) !important;
    ...
    #${FLYOUT_ID} .settings-brand-card {
      ...
      padding: 12px;
      background:
        linear-gradient(90deg, rgba(163, 51, 255, 0.12), rgba(30, 35, 40, 0.65)),
        rgba(1, 10, 19, 0.72);
    ...
    #${FLYOUT_ID} .settings-health-card {
      ...
      border: 1px solid rgba(163, 51, 255, 0.76);
      box-sizing: border-box;
    }
    ...
    #${FLYOUT_ID} .aurelia-tooltip-wrapper {
      display: inline-flex;
      align-items: center;
    ...
    #${FLYOUT_ID} .aurelia-tooltip-icon {
      width: 14px;
      height: 14px;
      background-image: url("http://127.0.0.1:${window.__aureliaBridge ? window.__aureliaBridge.port : 50000}/asset/tooltip.png");
    ...
    #${FLYOUT_ID} .aurelia-tooltip-icon:hover {
      opacity: 1;
    }

    #${FLYOUT_ID} .aurelia-tooltip-icon:focus-visible {
      outline: 1px solid #a333ff;
      outline-offset: 2px;
      border-radius: 3px;
    }

    /* Tooltip bubble is rendered globally (outside flyout) */
    #aurelia-global-tooltip {
      position: fixed;
      left: 0;
      top: 0;
      width: 340px;
    ...
    #aurelia-global-tooltip[data-show="true"] {
      opacity: 1;
      visibility: visible;
      transform: translateY(0px);
    }

    #aurelia-global-tooltip::after {
      content: "";
      position: absolute;
      left: var(--aurelia-tooltip-arrow-x, 50%);
    ...
    #aurelia-global-tooltip[data-placement="top"]::after {
      top: 100%;
      border-top: 7px solid #0b1a2a;
    }

    #aurelia-global-tooltip[data-placement="top"]::before {
      top: 100%;
    ...
    #aurelia-global-tooltip[data-placement="bottom"]::after {
      top: -7px;
      border-bottom: 7px solid #0b1a2a;
    }

    #aurelia-global-tooltip[data-placement="bottom"]::before {
      top: -8px;
    ...
    #skin-selection-flyout .lc-flyout-content {
      ...
      width: 100% !important;
      box-sizing: border-box !important;
      border: 1px solid #a333ff !important;
    ...
    .champion-card:hover {
      border-color: #a333ff;
      background: rgba(163, 51, 255, 0.08);
    }
    .champion-card img {
      width: 60px;
      height: 60px;
    ...
    .skin-card:hover {
      border-color: #a333ff;
      box-shadow: 0 0 8px rgba(163, 51, 255, 0.3);
    }
    .skin-card img {
      width: 100%;
  ...
  function handleSettingsData(payload) {
    ...
    const badge = document.getElementById("aurelia-version-badge");
  ...
  function startBadgeObserver() {
    ...
    // Re-apply badges when the Golden Aurelia nav item is injected by ROSE-UI (or recreated by Ember).
    const tryApply = () => {
      try {
        applyErrorBadges();
      } catch (e) {}
  ...
  function applyErrorBadges() {
    // Sidebar "Golden Aurelia" nav icon badge
    const navItem = document.querySelector(
      "lol-uikit-navigation-item.menu_item_Golden.Aurelia"
    );
    if (navItem) {
      const host =
      ...
      // Use warning image overlay (assets/red-warning.png) on the top-right of the Aurelia icon.
      let badge = host.querySelector("#aurelia-errors-badge");
      if (errorBadgeState.hasErrors) {
        if (!badge) {
          badge = document.createElement("div");
          badge.id = "aurelia-errors-badge";
          badge.classList.add("aurelia-warning-glow");
          ...
          badge.style.backgroundImage = `url(http://127.0.0.1:${window.__aureliaBridge ? window.__aureliaBridge.port : 50000}/asset/red-warning.png)`;
    ...
    if (tb) {
      ...
      let warn = tb.querySelector("#aurelia-troubleshoot-warning");
      if (errorBadgeState.hasErrors) {
        if (!warn) {
          warn = document.createElement("div");
          warn.id = "aurelia-troubleshoot-warning";
          warn.classList.add("aurelia-warning-glow");
          ...
          warn.style.backgroundImage = `url(http://127.0.0.1:${window.__aureliaBridge ? window.__aureliaBridge.port : 50000}/asset/red-warning.png)`;
  ...
  function createSettingsFlyout(navItem) {
    ...
    function getOrCreateGlobalTooltip() {
      let el = document.getElementById("aurelia-global-tooltip");
      if (el) return el;

      el = document.createElement("div");
      el.id = "aurelia-global-tooltip";
    ...
    function hideGlobalTooltip() {
      const el = document.getElementById("aurelia-global-tooltip");
      if (!el) return;
      el.setAttribute("data-show", "false");
    }
    ...
    function showGlobalTooltipFor(anchorEl, text) {
      ...
      tooltip.style.setProperty("--aurelia-tooltip-arrow-x", `${Math.round(arrowX)}px`);
    }

    function createTooltipButton(tooltipText, ariaLabel) {
      const wrapper = document.createElement("span");
      wrapper.className = "aurelia-tooltip-wrapper";

      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "aurelia-tooltip-icon";
      ...
      const reposition = () => {
        const tt = document.getElementById("aurelia-global-tooltip");
    ...
    versionBadge.id = "aurelia-version-badge";
    ...
    thresholdFill.style.background = "linear-gradient(to left, #a333ff, #a333ff)";
    ...
    benchmarkInfo.id = "aurelia-threshold-benchmark";
    ...
    timeoutFill.style.background = "linear-gradient(to left, #a333ff, #a333ff)";
    ...
    const injectShadowStyles = () => {
      ...
      if (root.querySelector('style[data-aurelia-dropdown-color]')) {
        return;
      }

      const rootStyle = document.createElement("style");
      rootStyle.setAttribute("data-aurelia-dropdown-color", "true");
    ...
    presetsSection.className = "settings-section";
    presetsSection.style.marginTop = "20px";
    presetsSection.style.borderTop = "1px solid #3c3c41";
    presetsSection.style.paddingTop = "12px";

    const presetsLabel = document.createElement("label");
    presetsLabel.className = "settings-label";
    ...
    presetsList.id = "aurelia-presets-list";
    presetsList.style.display = "flex";
    presetsList.style.flexWrap = "wrap";
    presetsList.style.gap = "8px";
    presetsList.style.marginBottom = "8px";
    ...
    presetNameInput.className = "settings-input";
    presetNameInput.placeholder = "Preset name...";
    presetNameInput.style.flex = "1";
    presetActions.appendChild(presetNameInput);

    const savePresetBtn = document.createElement("lol-uikit-flat-button-secondary");
    savePresetBtn.textContent = "Save Preset";
    savePresetBtn.style.width = "auto";
    savePresetBtn.addEventListener("click", () => {
      const name = presetNameInput.value.trim() || "Default";
      if (bridge && window.__aureliaLastHoveredSkin) {
        bridge.send({
          type: "preset-save",
          championId: window.__aureliaLastHoveredSkin.championId,
          name: name,
          skinId: window.__aureliaLastHoveredSkin.skinId,
          chromaId: window.__aureliaLastHoveredSkin.chromaId,
          formPath: window.__aureliaLastHoveredSkin.formPath,
        });
        presetNameInput.value = "";
      } else {
        log("warn", "No skin selected to save as preset");
      }
    });
    presetActions.appendChild(savePresetBtn);
    presetsSection.appendChild(presetActions);
    form.appendChild(presetsSection);

    function renderPresets() {
      const list = document.getElementById("aurelia-presets-list");
      if (!list) return;
      list.innerHTML = "";
      const presets = window.__aureliaPresets || {};
    ...
    const updateHoverEffects = () => {
      if (isHovered || isDragging) {
        fill.style.background = isDragging
          ? 'linear-gradient(to right, #a333ff, #a333ff)'
          : 'linear-gradient(to right, #a333ff 0%, #00f2ff 56%, #a333ff 100%)';
        button.style.backgroundPosition = isDragging ? '0 -60px' : '0 -30px';
      } else {
        fill.style.background = 'linear-gradient(to left, #a333ff, #a333ff)';
  ...
  function updateSettingsForm() {
    ...
    const versionBadge = document.getElementById("aurelia-version-badge");
  ...
  function openChampionSelection() {
    ...
    loadingIndicator.style.color = "#00f2ff";
    ...
    searchInput.addEventListener("input", (e) => {
      ...
      const allChampions = window.__aureliaAllChampions || [];
    ...
    window.__aureliaChampionRenderer = renderChampionsGrid;
  }
  ...
  function closeChampionSelection() {
    ...
    delete window.__aureliaChampionRenderer;
    delete window.__aureliaAllChampions;
  }
  ...
  function renderChampionsGrid(champions) {
    ...
    if (champions.length === 0) {
      championsGrid.innerHTML = `<div style="grid-column: 1 / -1; color: #00f2ff; text-align: center; padding: 20px; font-family: 'Beaufort for LOL', serif;">No champions found matching your search.</div>`;
      return;
    }
  ...
  function openSkinSelection(championId) {
    ...
    window.__aureliaSelectedChampionId = championId;
  }
  ...
  function closeSkinSelection() {
    ...
    delete window.__aureliaSelectedChampionId;
  }
  ...
  function handleChampionsListResponse(payload) {
    ...
    if (champions.length === 0) {
      championsGrid.innerHTML = `<div style="color: #00f2ff; text-align: center; padding: 20px; font-family: 'Beaufort for LOL', serif;">No champions found. Please ensure League of Legends client is running.</div>`;
      return;
    }

    // Store champions for search functionality
    window.__aureliaAllChampions = champions;

    // Render champions
    if (window.__aureliaChampionRenderer) {
      window.__aureliaChampionRenderer(champions);
    } else {
  ...
  function handleChampionSkinsResponse(payload) {
    ...
    if (skins.length === 0) {
      skinsListContainer.innerHTML = `<div style="color: #00f2ff; text-align: center; padding: 20px; font-family: 'Beaufort for LOL', serif;">No skins found for this champion.</div>`;
      return;
    }
  ...
  function openDiagnosticsDialog() {
    ...
    const existing = document.getElementById("aurelia-diagnostics-dialog");
    if (existing) {
      existing.remove();
    ...
    const dialog = document.createElement("div");
    dialog.id = "aurelia-diagnostics-dialog";
    dialog.style.position = "fixed";
    dialog.style.top = "0";
    dialog.style.left = "0";
    dialog.style.width = "100%";
    ...
    panel.style.border = "1px solid #a333ff";
    ...
    body.id = "aurelia-diagnostics-body";
    body.style.color = "#00f2ff";
    ...
    foot.id = "aurelia-diagnostics-foot";
  ...
  function renderDiagnosticsDialog() {
    ...
    const body = document.getElementById("aurelia-diagnostics-body");
    const foot = document.getElementById("aurelia-diagnostics-foot");
    if (!body || !foot) return;
    ...
    if (errors.length === 0) {
      ...
      const describe = (e) => {
        ...
        if (isInjectionThreshold) {
          ...
          let fixText;
          if (thresholdAtMax) {
            fixText = `Fix: you're already at the maximum Injection Threshold. This usually means the injection is extremely slow. Try lighter mods, close heavy apps, move League/mods to an SSD, and consider adding antivirus exclusions for the League and Aurelia folders. Then retry.`;
        ...
        if (isMonitorTimeout) {
          ...
          return {
            ...
            details: [
              `What it means: injection took longer than the allowed time, so AURELIA stopped the process.`,
              timeoutAtMax
                ? `Fix: you're already at the maximum Monitor Auto-Resume Timeout. This usually means the injection is extremely slow. Try lighter mods, close heavy apps, move League/mods to an SSD, and consider adding antivirus exclusions for the League and Aurelia folders. Then retry.`
          ...
          return `
            <div style="border:1px solid rgba(70,55,20,0.55); background: rgba(1,10,19,0.35); padding:8px; margin-bottom:8px;">
              <div style="display:flex; gap:8px; align-items:baseline; margin-bottom:6px;">
                <span style="font-weight:800; color:#00f2ff;">${idx + 1}.</span>
                ${tsHtml}
  ...
  function renderThresholdBenchmark() {
    const el = document.getElementById("aurelia-threshold-benchmark");
    if (!el) return;
    ...
    let html;
    if (needsIncrease) {
      html = `<span style="color:#a333ff;">Based on ${label}, we recommend <span style="color:#00f2ff; font-weight:700;">${recS}s</span></span>`;
      html += ` <button id="aurelia-apply-recommended-btn" style="
        margin-left:4px; padding:1px 8px; border:1px solid #a333ff; background:#1e2328;
        color:#00f2ff; cursor:pointer; font-family:'Beaufort for LOL',serif; font-size:11px;
    ...
    const applyBtn = document.getElementById("aurelia-apply-recommended-btn");
    if (applyBtn) {
  ...
  function closeSettingsPanel() {
    ...
    const lastActiveNavItem = document.querySelector(".main-nav-bar > * > lol-uikit-navigation-item[aureliaLastActive]");
    if (lastActiveNavItem) {
      lastActiveNavItem.removeAttribute("aureliaLastActive")
    ...
    try {
      const diag = document.getElementById("aurelia-diagnostics-dialog");
      if (diag) diag.remove();
      diagnosticsDialog = null;
    } catch (e) {}
  ...
  window.addEventListener("aurelia-open-settings", (e) => {
    const navItem =
      e.detail?.navItem ||
    ...
    if (navItem) {
      ...
      log(
        "warn",
        "Could not find Golden Aurelia nav item to position settings panel"
      );
    }
  });
  ...
  function injectCSS() {
    ...
    const existingStyle = document.getElementById("aurelia-settings-panel-css");
    if (existingStyle) {
      existingStyle.remove();
    }

    const style = document.createElement("style");
    style.id = "aurelia-settings-panel-css";
  ...
  async function init() {
    ...
    try {
      ...
      bridge.subscribe("presets-data", (payload) => {
        window.__aureliaPresets = payload.presets;
        renderPresets();
      });
      bridge.subscribe("preset-saved", (payload) => {
        if (payload.success) {
          if (bridge) bridge.send({ type: "presets-request", championId: payload.championId });
        }
      });
      bridge.subscribe("preset-deleted", (payload) => {
        if (payload.success) {
          if (bridge) bridge.send({ type: "presets-request", championId: payload.championId });
        }
      });
